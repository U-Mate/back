...
const createReview = async (req, res) => {
    ...
    try {
        ...
        if(rating < 0 || rating > 5){
            await conn.rollback();
            conn.release();
            console.error("리뷰 평점은 0~5 사이의 값이어야 합니다.");
        ...
        if(review.length < 10 || review.length > 100){
            await conn.rollback();
            conn.release();
            console.error("리뷰 내용은 10~100자 사이의 값이어야 합니다.");
            return res.status(404).json({success : false, message : "비정상적인 접근입니다."});
        }
...
const updateReview = async (req, res) => {
    ...
    try {
        ...
        await conn.commit();
        conn.release();
        console.log(`${reviewId} 리뷰 수정 성공`);
        return res.status(200).json({success : true, message : "리뷰 수정 성공"});
    } catch (error) {
        await conn.rollback();
        conn.release();
        console.error(error);
        return res.status(500).json({error : "리뷰 수정 중 오류가 발생했습니다."});
    }
}
...
const deleteReview = async (req, res) => {
    ...
    try {
        ...
        await conn.commit();
        conn.release();
        console.log(`${reviewId} 리뷰 삭제 성공`);
        return res.status(200).json({success : true, message : "리뷰 삭제 성공"});
    } catch (error) {
        await conn.rollback();
        conn.release();
        console.error(error);
        return res.status(500).json({error : "리뷰 삭제 중 오류가 발생했습니다."});
    }
}
